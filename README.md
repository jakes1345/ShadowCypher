...
```mermaid
graph TD
    subgraph "Tactical Layer (Native GTK3)"
        UI[Sovereign HUD] --> BUS[ShadowBus / shm]
    end

    subgraph "Directive Core (Obsidian Citadel)"
        BUS --> HUB[ShadowHub / Singleton]
        HUB --> SIG[ShadowPulse SIGINT Analyzer]
        HUB --> AI[Autonomous Engine Cluster]
        AI --> SYN[DeepHat Synthesis Engine]
    end

    subgraph "Polyglot Execution Platform (V4.5.9)"
        HUB --> RUN[Apex Runner]
        RUN --> P2[Runtime: Python 2.7 / Legacy Bridge]
        RUN --> P3[Runtime: Python 3.12 / Modern Core]
        RUN --> NB[Runtime: Native Binary / C / Go / Rust]
    end

    subgraph "Target Engagement"
        P3 --> EX[AI Forged Exploit]
        P2 --> LR[Classic Responder / NTLM Relay]
