    ...
    def deep_leak_correlation(self, target_email, on_output=None):
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=10) as resp:
                ...
                if on_output:
                    on_output(f"[BREACH] {len(data)} breach(es) found for {target_email}:\n")
                    for b in data:
                        on_output(f"  • {b['Name']} ({b['BreachDate']}) — {b['DataClasses']}\n")
        except urllib.error.HTTPError as e:
            if e.code == 404:
                if on_output: on_output(f"[CLEAN] No breaches found for {target_email}\n")
            elif e.code == 401:
                if on_output: on_output("[WARN] HIBP API key required for email lookups — use domain endpoint instead\n")
            else:
                if on_output: on_output(f"[ERROR] HIBP API error {e.code}: {e}\n")
        except Exception as ex:
            if on_output: on_output(f"[ERROR] Breach lookup failed: {ex}\n")
