...
class TacticalDatabase:
    def __init__(self):
        self.db_path = os.path.join(config.project_root, 'projects', 'shadow_registry.db')
    ...
    def register_target(self, ip, hostname="UNKNOWN", mac="UNKNOWN"):
        """Called automatically by Kairos when an IP is detected."""
        now = datetime.now().isoformat()
        try:
            self.cursor.execute('''
                INSERT INTO target_grid (ip_address, hostname, mac_address, first_seen, last_scanned)
                VALUES (?, ?, ?, ?, ?)
            ''', (ip, hostname, mac, now, now))
            self.conn.commit()
            logger.info("database", f"Target {ip} Locked in SQLite Matrix.")
    ...
    def log_vulnerability(self, ip, port, service, cve_id="N/A", severity="MEDIUM", payload=""):
        self.register_target(ip) # Auto-create if not exists
        self.cursor.execute('''
            INSERT INTO vulnerability_graph (target_ip, port, service, cve_id, severity, payload_notes)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (ip, port, service, cve_id, severity, payload))
        self.conn.commit()

    def export_target_graph(self):
        """Used by the HUD and Tengu AI to build active network topologies."""
