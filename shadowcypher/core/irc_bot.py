...
def cmd_deephat(nick, target, args, reply, admin):
    """Invoke DeepHat Apex for tool synthesis."""
    if not args:
        return reply("DEEPHAT_ERROR: Usage: !deephat <type> <description>")
    if not admin:
        return reply("ACCESS_DENIED: DeepHat requires Operator Clearance.")

    parts = args.split(" ", 1)
    category = parts[0].lower()
    desc = parts[1] if len(parts) > 1 else ""

    reply(f"\u26a0\ufe0f FORGE_STARTING: Type={category.upper()} Target=\"{desc}\"")
    try:
        from shadowcypher.modules.deephat import deephat
        filename = deephat.forge_weapon(desc, category=category)
        reply(f"\u2705 WEAPON_READY: {filename}")
    except Exception as e:
        reply(f"DEEPHAT_FAILURE: {e}")

@hook.command("execute")
def cmd_execute(nick, target, args, reply, admin):
    """Execute a forged payload."""
    if not args:
