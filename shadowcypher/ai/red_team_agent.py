...
Usage:
    ...
    mission_id = red_team.engage("192.168.1.100", on_output=callback)
    red_team.stop(mission_id)
"""
...
class AutonomousRedTeamAgent:
    ...
    def engage(
        ...
        def _run():
            try:
                ...
                try:
                    ...
                    logger.warning("red_team", f"KG ingest failed: {e}")
            except Exception as e:
                if on_output:
                    on_output(f"[RED_TEAM] MISSION_CRASH: {e}\n")
                logger.error("red_team", f"Mission {mission_id} crashed: {e}")
            finally:
                ctx.complete = True
        ...
        logger.info("red_team", f"MISSION_START: {mission_id} → {target}")
        return mission_id
    ...
    def stop(self, mission_id: str):
        ...
        with self._lock:
            ...
            if flag:
                flag.set()
                logger.info("red_team", f"MISSION_ABORT: {mission_id}")
...
Be specific. Reference actual CVE IDs and port numbers. No filler text."""
        ...
        try:
            ...
            logger.warning("red_team", f"AI synthesis failed: {e} — using fallback")
            return self._fallback_report(ctx)
    ...
    def _save_report(self, ctx: MissionContext, report: str) -> str:
        ...
        try:
            with open(path, "w") as f:
                f.write(report)
            os.chmod(path, 0o600)
            logger.info("red_team", f"REPORT_SAVED: {path}")
        except Exception as e:
            logger.warning("red_team", f"Report save failed: {e}")
            path = ""
        return path
...
# Global singleton
red_team = AutonomousRedTeamAgent()
