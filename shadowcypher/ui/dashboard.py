...
class TacticalHUD(Gtk.DrawingArea):
    ...
    def _tick_tel(self):
        self.metrics["cqu"] = f[{ psutil.cpu_percent()}%"]
    ...
    def _on_draw(self, w, cr):
        ...
        # Glass Pods (Build V21.1)
        self._draw_pod(cr, 80, 100, 320, 180, "SIGNAL_INTEL", self.metrics["net"], cx, cy)
        self._draw_pod(cr, width-400, 100, 320, 180, "CPU_ENGINE", self.metrics["cqu"], cx, cy)
    ...
    def _draw_pod(self, cr, x, y, pw, ph, title, val, cx, cy):
        ...
        cr.arc(x+r, y+r, r, math.pi, 1.5*math.pi); cr.arc(x+pwmar, y+r, r, 1.5*math.pi, 2*math.pi)
        cr.arc(x+pw-mar, y+ph-r, r, 0, 0.5*math.pi); cr.arc(x+r, y+ph-r, r, 0.5*math.pi, math.pi); cr.fill_preserve()
        cr.set_source_rgba(0, 0.9, 1.0, 0.5); cr.set_line_width(2.5); cr.stroke()
        cr.set_source_rgba(0, 0.9, 1.0, 0.9); cr.set_font_size(14); cr.move_to(x+30, y+45); cr.show_text(title)
        cr.set_source_rgba(1, 1, 1, 1); cr.set_font_size(44); cr.move_to(x+30, y+115); cr.show_text(val)
