...
async function mintVapidJwt(env: Env, audience: string): Promise<string> {
  ...
  // Standard VAPID private keys (generated by web-push or vapidgen) are raw 32-byte
  // EC scalars encoded as base64url — NOT PKCS#8. Import as JWK to get a signing key.
  const rawPrivBytes = b64urlDecode(env.PUSH_VAPID_PRIVATE_KEY);
  const rawPrivB64u = b64urlEncode(rawPrivBytes);
  // Also need the public key to build the JWK
  const rawPubBytes = b64urlDecode(env.PUSH_VAPID_PUBLIC_KEY);
  // Uncompressed P-256 public key is 65 bytes: 0x04 || X(32) || Y(32)
  const x = b64urlEncode(rawPubBytes.slice(1, 33));
  const y = b64urlEncode(rawPubBytes.slice(33, 65));
  const jwk = {
    kty: "EC",
    crv: "P-256",
    x,
