...
class KnowledgeGraph:
    ...
    def add_cve(self, cve_id: str, score: float = 0.0, severity: str = "",
                desc: str = "", **props):
        self.add_node(cve_id, "CVE", props={"score": score, "severity": severity,
                                             "description": desc[:200], **props})

    def add_service(self, service_id: str, port: int = 0, proto: str = "",
                    version: str = "", **props):
        self.add_node(service_id, "SERVICE",
                      props={"port": port, "proto": proto, "version": version, **props})

    def add_credential(self, cred_id: str, username: str = "", source: str = "",
                       hash_type: str = "", **props):
        self.add_node(cred_id, "CREDENTIAL",
                      props={"username": username, "source": source,
                             "hash_type": hash_type, **props})

    def add_mission(self, mission_id: str, target: str = "", stage: str = "", **props):
        self.add_node(mission_id, "MISSION",
                      props={"target": target, "stage": stage, **props})

    # ── Edge Operations ──────────────────────────────────────────────────────

    def link(self, src: str, dst: str, rel: str, weight: float = 1.0,
             **props) -> int:
    ...
    def unlink(self, src: str, dst: str, rel: Optional[str] = None):
        ...
        with self._lock, self._conn() as c:
            if rel:
                c.execute("DELETE FROM edges WHERE src=? AND dst=? AND rel=?",
                          (src, dst, rel.upper()))
            else:
    ...
    def find_by_type(self, node_type: str, limit: int = 100) -> list[dict]:
        with self._conn() as c:
            rows = c.execute(
                "SELECT * FROM nodes WHERE type=? ORDER BY updated DESC LIMIT ?",
                (node_type.upper(), limit)
            ).fetchall()
        ...
        with self._conn() as c:
            while queue:
                ...
                if len(path) >= max_depth:
                    continue
                rows = c.execute(
                    "SELECT dst FROM edges WHERE src=?", (node,)
                ).fetchall()
                for row in rows:
                    nxt = row["dst"]
    ...
    def ingest_red_team_mission(self, ctx) -> int:
        ...
        for p in ctx.open_ports:
            ...
            self.add_service(svc_id, port=p["port"], proto=p["proto"],
