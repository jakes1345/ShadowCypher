...
class StegoEngine:
    ...
    def hide(self, payload, cover_path: str, output_path: str,
        ...
        try:
            ...
            for r, g, b in pixels:
                ...
                new_px.append((r, g, b))
            out = Image.new("RGB", img.size); out.putdata(new_px)
            if not output_path.endswith(".png"): output_path += ".png"
            out.save(output_path, "PNG"); os.chmod(output_path, 0o600)
    ...
    def reveal(self, stego_path: str, key: bytes = None, on_output=None) -> Optional[bytes]:
        ...
        try:
            ...
            if key and HAS_CRYPTO:
                aesgcm = AESGCM(key); raw = aesgcm.decrypt(raw[:12], raw[12:], None)
    ...
    def capacity(self, path: str) -> int:
        ...
        try: img = Image.open(path); return (img.width * img.height * 3) // 8
